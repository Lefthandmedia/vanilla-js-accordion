...
class Accordion {
  constructor(el, options) {
    ...
    var defaults = {
      ...
      setFocus: "item", // options: none, item, panel, target, control, first
      hashEnabled: true,
    };
  ...
  createItems() {
    ...
    return itemArray.map(function (item, i) {
      ...
      var control =
        self.opts.target === self.opts.control
          ? target
          : el.querySelector(self.opts.control);
      ...
      if (!attribute) {
        el.setAttribute(
          self.opts.attribute,
          isExpanded ? self.opts.expanded : self.opts.contracted
        );
      }
